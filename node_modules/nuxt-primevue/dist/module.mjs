...
const composables = [{ name: "useStyle", as: "useStyle", from: "primevue/usestyle" }];
...
function registerItems(items = [], options = {}, params) {
  ...
  return items.filter((item) => {
    ...
    const matchedIn = included === "*" || included === void 0 ? true : Utils.object.isNotEmpty(included) ? included.some((inc) => name?.toLowerCase() === inc.toLowerCase()) : false;
...
function register(options) {
  const resolvePath = (resolveOptions) => Utils.object.getPath(options.resolvePath, resolveOptions);
  const config = registerConfig(resolvePath);
  const components2 = registerComponents(resolvePath, options.components);
  const directives2 = registerDirectives(resolvePath, options.directives);
  const composables2 = registerComposables(resolvePath, options.composables);
  const registered = {
