...
import { initiateChart, getRangeIndexes, isNumber, isDateTime, getSheetIndexFromAddress, checkDateFormat } from '../../workbook/common/index';
import { overlay, locale, refreshChartCellObj, getRowIdxFromClientY, getColIdxFromClientX, deleteChart, dialog, overlayEleSize, undoRedoForChartDesign, addDPRValue } from '../common/index';
...
var SpreadsheetChart = /** @class */ (function () {
        ...
        while (minr <= maxr) {
            ...
            while (minc <= maxc) {
                ...
                if (cell.format && !isYvalue) {
                    var forArgs = { value: cell.value, format: cell.format, formattedText: cell.value, rowIndex: minr,
        ...
        while (i < diff) {
            ...
            while (j < len) {
                if (yValue[yInc]) {
                    ...
                    if (isNumber(val)) {
                        ...
                        var dateCheck = { value: isNullOrUndefined(val) ? '' : val.toString(), sheetIndex: sheetIndex, updatedVal: '' };
            ...
            if (options.type) {
                ...
                if (type === 'Line' || type === 'StackingLine' || type === 'StackingLine100') {
                    pObj = {
                        dataSource: pArr, type: options.type, xName: 'x', yName: 'y', name: seriesName.toString(),
    ...
    SpreadsheetChart.prototype.initiateChartHandler = function (argsOpt) {
        ...
        var overlayProps = overlayObj.insertOverlayElement(id, eleRange, getSheetIndexFromAddress(this.parent, eleRange));
        ...
        var format = this.primaryYAxisFormat(yRange);
        ...
        if (chart.type !== 'Pie' && chart.type !== 'Doughnut') {
            this.chart = new Chart({
                ...
                axisLabelRender: function (args) {
                    if (args.axis.name === 'primaryYAxis' && format && !isNullOrUndefined(args.value) && _this.parent) {
                        args.text = _this.parent.getDisplayText({ format: format, value: args.value.toString() });
                    }
    ...
    SpreadsheetChart.prototype.deleteChart = function (args) {
        ...
        var eventArgs = {
            id: chartObj.id, range: chartObj.range, type: chartObj.type, markerSettings: chartObj.markerSettings, theme: chartObj.theme, height: chartObj.height,
    ...
    SpreadsheetChart.prototype.chartDesignTabHandler = function (args) {
        ...
        if (args.triggerEvent) {
            var eventArgs = { switchRowColumn: args.switchRowColumn, chartType: args.chartType, chartTheme: args.chartTheme,
        ...
        if (args.triggerEvent && !opensTitleDialog) {
            var eventArgs = { switchRowColumn: args.switchRowColumn, chartType: args.chartType, chartMarker: args.markerVisible, chartTheme: args.chartTheme,
    ...
    SpreadsheetChart.prototype.switchRowColumn = function (chartCollId, chartId, chartComp, cell) {
        ...
        for (var idx = 0, chartCount = cell.chart.length; idx < chartCount; idx++) {
            if (cell.chart[idx].id === chartId) {
                cell.chart[idx].isSeriesInRows = this.parent.chartColl[chartCollId].isSeriesInRows;
            }
        }
        var chartSeries = this.initiateChartHandler({ option: this.parent.chartColl[chartCollId], isRefresh: true, isSwitchRowColumn: true });
        chartComp.series = chartSeries;
    };
