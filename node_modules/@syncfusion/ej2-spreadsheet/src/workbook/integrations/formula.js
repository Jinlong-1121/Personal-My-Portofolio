import { getSheetName, getSheet, getSheetIndexFromId } from '../index';
...
var WorkbookFormula = /** @class */ (function () {
    ...
    WorkbookFormula.prototype.definedNamesDeletion = function (sheetName) {
        ...
        if (definedNames && definedNames.length > 0) {
            for (var i = definedNames.length - 1; i >= 0; i--) {
                if (definedNames[i].refersTo.split('=')[1].split('!')[0].split("'").join('') === sheetName) {
    ...
    WorkbookFormula.prototype.refreshCalculate = function (rowIdx, colIdx, value, isFormula, sheetIdx, isRefreshing, isDependentRefresh, isRandomFormula) {
        ...
        if (isFormula) {
            ...
            var updatedCell = getCell(rowIdx, colIdx, sheet);
            if (updatedCell && formula && !isDependentRefresh) {
                formula = formula.toString().toUpperCase();
                var formulaStr = formula.indexOf('=SUM(') === 0 ? '=SUM(' : formula.indexOf('=AVERAGE(') === 0 ? '=AVERAGE(' : '';
                if (formulaStr) {
                    ...
                    for (var idx = 0; idx < cellRefArr.length; idx++) {
                        ...
                        if (cellRef_1.includes('!')) {
                            refArr = cellRef_1.split('!');
                            sheetName_1 = refArr[0];
                            cellRef_1 = refArr[1];
                        }
                        else {
                            sheetName_1 = '';
                        ...
                        if (isCellReference(cellRef_1)) {
                            if (sheetName_1) {
                                sheetIdx_1 = getSheetIndex(this.parent, sheetName_1);
                                model = sheetIdx_1 !== undefined ? getSheet(this.parent, sheetIdx_1) : sheet;
                            }
                            else {
                                model = sheet;
                            }
                            ...
                            if (!format_1) {
                                ...
                                if (isCellReference(cellRef)) {
                                    ...
                                    if (family_1.tokenToParentObject.has(sheetRef_1)) {
                                        sheetIdx_2 = getSheetIndexFromId(_this.parent, Number(family_1.tokenToParentObject.get(sheetRef_1)));
                                        model_1 = sheetIdx_2 !== undefined ? getSheet(_this.parent, sheetIdx_2) : sheet;
                                    }
                                    else {
                                        model_1 = sheet;
                                    }
                                    ...
                                    if (fCell_1 && fCell_1.format) {
                                        format_1 = fCell_1.format;
                                    }
                                }
                            }
                        });
                        if (format_1) {
                            updatedCell.format = format_1;
                        }
                    }
                }
            }
