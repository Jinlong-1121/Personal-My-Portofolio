            ...
            while (1 < operators.length) {
                switch (operators.substring(0, 2)) {
                    ...
                    case '&-':
                        ...
                        break;
                    default:
                        throw new FormulaError(_this.parent.formulaErrorStrings[FormulasErrorsStrings.invalid_expression], true);
                }
            }
        ...
        if (this.parent.formulaErrorStrings.indexOf(formula) > -1) {
            ...
            while (i < formula.length) {
                ...
                if ((this.parent.isDigit(formula[i]) && ((formula.length > i + 1)
                    ...
                    formula[i] === this.sheetToken || formula[i] === '$' || formula[i] === '_') {
                    form = form + formula[i];
                    i = i + 1;
                }
                else {
                    ...
                    if (!isNotOperator) {
                        ...
                        if (text[j] === this.parent.arithMarker) {
                            ...
                            while (j > -1 && (this.parent.isDigit(text[j]) ||
                                (!period && (text[j] === this.parent.getParseDecimalSeparator() || text[j] === '%')))) {
                                if (!this.parent.isDigit(text[j])) {
                                    period = true;
                                }
                                j = j - 1;
                            }
