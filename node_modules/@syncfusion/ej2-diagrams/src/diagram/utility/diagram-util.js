...
import { ConnectorConstraints, NodeConstraints, PortConstraints, DiagramConstraints, DiagramTools, Transform, BlazorAction, ControlPointsVisibility } from './../enum/enum';
...
function getAnnotation(obj, element) {
    ...
    for (j = 0; annotations && j < annotations["" + length]; j++) {
        if (element.annotationId && annotations[parseInt(j.toString(), 10)].id === element.annotationId) {
...
export function getDropEventArguements(args, arg) {
    if (isBlazor()) {
        var isConnector = (getObjectType(args.source) === Connector);
