...
import { BlockWidget, ParagraphWidget, LineWidget, BodyWidget, TableCellWidget, FieldElementBox, TableWidget, TableRowWidget, BookmarkElementBox, HeaderFooterWidget, CheckBoxFormField, TextFrame, TextElementBox, FootnoteElementBox, ImageElementBox } from '../viewer/page';
                ...
                if (node instanceof TableRowWidget) {
                    ...
                    if (node instanceof ParagraphWidget && !node.isInsideTable && this.action === 'RemoveRowTrack') {
                        ...
                        if (!isNullOrUndefined(node.lastChild) && node.lastChild instanceof LineWidget) {
                            this.owner.selection.start.setPositionParagraph(node.lastChild, node.lastChild.getEndOffset());
                        }
                    }
                    else if (node instanceof TableWidget && this.action === 'RemoveRowTrack') {
                        var block_1 = this.owner.selection.start.paragraph;
                        if (block_1.nextRenderedWidget && block_1.nextRenderedWidget instanceof TableWidget) {
                            var table = block_1.nextRenderedWidget;
                            node = node.combineWidget(this.viewer);
                            this.owner.editorModule.insertTableInternal(table, node, false);
                        }
                    }
                    else {
